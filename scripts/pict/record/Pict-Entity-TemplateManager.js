...
//		 * 	OPERATION Templates
			...
			_MetaTemplates.SecondaryRecordRender = loadMetaScript('SecondaryRecordRender');
			_MetaTemplates.RecordCommonDataRender = loadMetaScript('RecordCommonDataRender');
		...
		var _Replace_Read_Title_Pre = /MTS_Read_Title_Pre_MTE/g;
		var _Replace_Read_Title = /MTS_Read_Title_MTE/g;

		var _Replace_Create_Title_Pre = /MTS_Create_Title_Pre_MTE/g;
		var _Replace_Create_Title = /MTS_Create_Title_MTE/g;

		var _Replace_Update_Title_Pre = /MTS_Update_Title_Pre_MTE/g;
		var _Replace_Update_Title = /MTS_Update_Title_MTE/g;
		var processMetaScript = function(pMetaTemplateParameters)
		{
			...
			var _Config = pMetaTemplateParameters.DataOperations.Config;

			// There are some mini templates stored here for ease of passing in from Pict:
			var getMiniTemplate = function(pContext, pTemplate, pDefault)
			{
				if (_Config.PictConfig.hasOwnProperty(pContext) && typeof(_Config.PictConfig[pContext][pTemplate]) !== 'undefined')
				{
					return _Config.PictConfig[pContext][pTemplate];
				}

				return typeof(pDefault) === 'string' ? pDefault : '';
			};
			...
			tmpResult = tmpResult.replace(_Replace_ReadList_Title, getMiniTemplate('List','PageTitle', _Config.DataSetHash));
			tmpResult = tmpResult.replace(_Replace_ReadList_Title_Pre, getMiniTemplate('List','PageTitlePrefix'));

			tmpResult = tmpResult.replace(_Replace_Read_Title, getMiniTemplate('Record','PageTitle', _Config.DataSetHash));
			tmpResult = tmpResult.replace(_Replace_Read_Title_Pre, getMiniTemplate('Record','PageTitlePrefix'));

			tmpResult = tmpResult.replace(_Replace_Create_Title, getMiniTemplate('Create','PageTitle', _Config.DataSetHash));
			tmpResult = tmpResult.replace(_Replace_Create_Title_Pre, getMiniTemplate('Create','PageTitlePrefix'));

			tmpResult = tmpResult.replace(_Replace_Update_Title, getMiniTemplate('Update','PageTitle', _Config.DataSetHash));
			tmpResult = tmpResult.replace(_Replace_Update_Title_Pre, getMiniTemplate('Update','PageTitlePrefix'));
		...
		var generateTemplates = function(pDataOperations)
		{
			...
			generateTemplate('SecondaryRecordRender', pDataOperations);
			generateTemplate('RecordCommonDataRender', pDataOperations);
