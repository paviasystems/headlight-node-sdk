	...
	function(libAsync)
	{
		function oNew(pDataOperations, pPict)
		{
			...
			var MarshalRecordFromUpdateForm = function(pForm)
			{
				...
				{
					// Ignore all built-in PICT controls in the forms
					if ($('#'+pForm[i].name).hasClass('pictSelect4'))
					{
						var tmpEntityValues = pForm[i].value.split('-');
						if ((tmpEntityValues.length > 3) && (tmpEntityValues[0] === 'PICT'))
							tmpRecord[pForm[i].name] = tmpEntityValues[3];
					}
					else if ((pForm[i].name.substring(0, 6) != 'CHECK_') && (pForm[i].name.substring(0, 5) != 'PICT_'))
					{
			...
			var ProcessUpdateForm = function()
			{
				...
				// If these aren't passed in, use the defaults.
				if (!tmpAPIUpdateURI)
				{
					tmpAPIUpdateURI = _Config.APIURIMap.Update;
				}
				if (!tmpAPIUpdateMethod)
				{
					tmpAPIUpdateMethod = 'PUT';
				}
