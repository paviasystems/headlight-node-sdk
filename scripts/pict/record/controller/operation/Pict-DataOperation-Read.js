...
(
	function()
	{
		function oNew(pDataOperations, pPict)
		{
			...
			// Read a record id X into DOM location y, without page wrapping
			var DisplayRecordDataOnly = function(pIDRecord, pDOMLocation)
			{
				ReadRecord(
					{
						IDRecord: pIDRecord,
						ProcessRecordFunction: function(pRecord)
						{
							// Render the record out without the outer trappings
							$(pDOMLocation).html(_.template($("#RecordRender_"+_Config.DataSetHash).html())({Record:pRecord, Pict:_Pict, DAL:_DataOperations}));
						}
					}
				);
			};
			...
			var oDataOperation = (
			{
				...
				DisplayRecordDataOnly: DisplayRecordDataOnly
			});
